...
function order(e, h, g, b, a, c) {
	...
	$("#order").bootstrapTable({
		method: "get",
		url: "http://localhost/admin/getOrderAllById/v1",
		dataType: "json",
		headers: {
			"Authorization": $.cookie("token")
		},
...
function onpay(b) {
	var a = "http://localhost/admin/remind2Pay/v1";
	total(a, b)
}
function onout(b) {
	var a = "http://localhost/admin/outGoods/v1";
	total(a, b)
}
function oncomm(b) {
	var a = "http://localhost/admin/reminderEvaluation/v1";
	total(a, b)
}
function total(a, b) {
	$.ajax({

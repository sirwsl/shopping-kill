...
var update = true;
var flag = true;
var url = "http://localhost/admin/getBlackListForPhone/v1";
var number = "";
$(function() {
	limit();
	$("#titles").html("手机黑名单");
...
function change(a) {
	$("#titles").html("手机号黑名单");
	if (a == 10) {
		url = "http://localhost/admin/getBlackListForIp/v1";
		flag = false;
		$("#titles").html("IP地址黑名单");
...
$("#search").click(function() {
	url = "http://localhost/admin/selectBlackListByPhone/v1";
	if (!flag) {
		url = "http://localhost/admin/selectBlackListByIp/v1"
	}
	number = $("#seNum").val();
	limit();
	$("#limitList").bootstrapTable("refresh")
...
function delonly(a) {
	...
	$.ajax({
		url: "http://localhost/admin/delBlackListById/v1",
		data: "id=" + a,
		type: "delete",
		dataType: "json",
		success: function(b) {
...
function deleteMs(a) {
	$.ajax({
		url: "http://localhost/admin/delBlackListByIds/v1",
		data: "ids=" + a,
		type: "delete",
		dataType: "json",
		success: function(b) {
...
function submits(c) {
	var b = "http://localhost/admin/addBlackListByPhone/v1";
	if (!flag) {
		b = "http://localhost/admin/addBlackListByIp/v1"
	}
	if (!update) {
		b = "http://localhost/admin/updateBlackListById/v1"
	}
	var c = $("#adress").val();
	var a = $("#startTime").val();
	var e = $("#endTime").val();

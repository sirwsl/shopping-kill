...
$(function() {
	goodsType("http://localhost/admin/getTypesAll/v1", null, null)
});
function serches() {
	var b = "http://localhost/admin/getTypesByNameAndId/v1";
	var c = $("#searchId").val();
...
function goodsType(b, e, a) {
	...
	$("#goodstype").bootstrapTable({
		...
		onEditableSave: function(h, i, f, g) {
			$.ajax({
				type: "put",
				url: "http://localhost/admin/updateTypes/v1",
				data: i,
				dataType: "JSON",
				success: function(j) {
...
function deltype(b) {
	...
	if (a == true) {
		$.ajax({
			type: "DELETE",
			url: "http://localhost/admin/delTypes/v1",
			data: {
				id: b
			},
			dataType: "JSON",

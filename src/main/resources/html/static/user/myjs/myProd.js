$.ajaxSetup({
			...
			function pinjia(flag){
				var html ='';
				$.get('http://localhost/user/getAppraisalAll/v1',{'flag':flag},function(msg){
		...
		$('#bao').click(function(){
			if ($("#bao").val()=="删除"){
				$.ajax({
				    type: 'delete',
				    url: "http://localhost/user/deleteAppraisal/v1",
				...
				$.post('http://localhost/user/addAppraisal/v1',{'orderId':$("#orderIds").val(),'detail':$("#ping").val(),'grade':ttt},function(msg){
					if (msg.code == 0){
						alert(msg.data);
						pinjia(true);
					}else{

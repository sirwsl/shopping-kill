$.ajaxSetup({
		...
		$(function(){
			...
			reload(key,flag);
			$.get('http://localhost/api/getRecommendedGoods/v1',{'size':'5'},function(msg){
				if (msg.code == 0){
					var html = '';
		...
		function reload(id,f){
			...
			$.get('http://localhost/api/getDetail/v1',{'id':id,'flag':f},function(msg){
		...
		function addcart(){
			...
			var msg = "请确认是否添加进您的收藏夹"; 
			 ...
			 $.post("http://localhost/user/addCart/v1", {"skuId":skuId,"num":$('#number').val()},function(msg) {
			   alert(msg.data);
			 });
			 }else{ 
			  return false; 
		...
		function buy(flag){
			...
			var num = $('#number').val();
			if (flag == 100){
				$.get('http://localhost/api/'+skuId+'/getUrl',function(msg){
					if (msg.code == 0 && msg.data.seconds > 0){
						$("#mask1").css("display","block");
						$("#mask2").css("display","block");
						$.post('http://localhost/user/killGoods/'+skuId+'/'+num+'/'+msg.data.md5+'',function(m){
							if (m.code == 0){
				...
				$("#mask1").css("display","block");
				$("#mask2").css("display","block");
				$.post('http://localhost/user/commonGoods/v1',{'skuId':skuId,'num':num},function(m){
					if (m.code == 0){

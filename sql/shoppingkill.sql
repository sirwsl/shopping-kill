...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='限时抢购活动表';
...
CREATE TABLE `t_activity_limit` (
  ...
  `del_flag` tinyint(1) DEFAULT '0' COMMENT '逻辑删除',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='SKU在秒杀时候限制上限数量';
...
) ENGINE=InnoDB AUTO_INCREMENT=44 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='管理员表';
...
CREATE TABLE `t_after_sales` (
  ...
  `type` int(11) DEFAULT NULL COMMENT '处理类型 （0-退货退款 1-换货 3-仅退款）',
  ...
  `del_flag` tinyint(1) DEFAULT '0' COMMENT '是否删除',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='售后记录表';
...
CREATE TABLE `t_appraisal` (
  ...
  `grade` int(11) DEFAULT '5' COMMENT '评价星级（1-5）',
...
CREATE TABLE `t_loggers` (
  ...
  `detail` varchar(255) DEFAULT NULL COMMENT '操作内容',
  `man_id` bigint(20) DEFAULT NULL COMMENT '操作人id',
  `type` int(11) DEFAULT NULL COMMENT '操作类型(0-用户 1-管理员)',
...
CREATE TABLE `t_sku` (
  ...
  `warn_num` int(11) DEFAULT '10' COMMENT '预警量（默认10）',
...
CREATE TABLE `t_subscriber` (
  ...
  `type` int(11) NOT NULL COMMENT '订阅类型(0-手机订阅 1-邮件订阅)',
...
CREATE TABLE `t_subscription_history` (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=45 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='订阅历史表';
...
CREATE TABLE `t_types` (
  ...
  `del_flag` tinyint(1) DEFAULT '0' COMMENT '逻辑删除',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='商品类别表';
...
CREATE TABLE `t_user` (
  ...
  `del_flag` tinyint(1) NOT NULL DEFAULT '0' COMMENT '是否删除',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `phone` (`phone`) USING BTREE COMMENT '手机号非重复',
  UNIQUE KEY `id_card` (`id_card`) USING BTREE COMMENT '身份证非重复',
  UNIQUE KEY `name` (`name`) USING BTREE COMMENT '账号非重复'

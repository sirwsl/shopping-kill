...
CREATE TABLE `t_activity_limit`  (
  ...
  `limit_num` int NULL DEFAULT 0 COMMENT '上限数量',
...
CREATE TABLE `t_admin`  (
  `id` bigint NOT NULL AUTO_INCREMENT COMMENT 'id',
  `name` varchar(50) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '账户',
...
) ENGINE = InnoDB AUTO_INCREMENT = 44 CHARACTER SET = utf8 COLLATE = utf8_general_ci COMMENT = '管理员表' ROW_FORMAT = Dynamic;
...
CREATE TABLE `t_advertise`  (
  ...
  `img_url` varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NULL DEFAULT NULL COMMENT '广告图片链接',
...
CREATE TABLE `t_after_sales`  (
  ...
  `type` int NULL DEFAULT NULL COMMENT '处理类型 （0-退货退款 1-换货 3-仅退款）',
...
CREATE TABLE `t_appraisal`  (
  ...
  `grade` int NULL DEFAULT 5 COMMENT '评价星级（1-5）',
...
CREATE TABLE `t_loggers`  (
  ...
  `man_id` bigint NULL DEFAULT NULL COMMENT '操作人id',
  `type` int NULL DEFAULT NULL COMMENT '操作类型(0-用户 1-管理员)',
...
CREATE TABLE `t_sku`  (
  ...
  `warn_num` int NULL DEFAULT 10 COMMENT '预警量（默认10）',
...
CREATE TABLE `t_subscriber`  (
  ...
  `type` int NOT NULL COMMENT '订阅类型(0-手机订阅 1-邮件订阅)',
...
CREATE TABLE `t_subscription_history`  (
  ...
  `real` tinyint(1) NOT NULL DEFAULT 0 COMMENT '是否成功(0-失败  1-成功)',
